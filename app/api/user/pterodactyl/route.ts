...
function generatePassword(length: number = 12): string {
  ...
  let password = ''
  password += lowercase.charAt(randomBytes[0] % lowercase.length)
  password += uppercase.charAt(randomBytes[1] % uppercase.length)
  password += numbers.charAt(randomBytes[2] % numbers.length)
  
  for (let i = 3; i < length; i++) {
    password += allChars.charAt(randomBytes[i] % allChars.length)
  }
  
  // Безопасная перетасовка
