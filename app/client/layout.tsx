...
export default function ClientLayout({ children }: { children: ReactNode }) {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Проверка блокировки ТОЛЬКО если мы не на странице /banned
        if (data.user.banned && typeof window !== 'undefined' && !window.location.pathname.startsWith('/banned')) {
          console.log('[Client Layout] User is banned, redirecting to /banned')
          router.push('/banned')
          return
        }
  ...
  const handleLogout = async () => {
    ...
    if (user?.banned) {
      notify.error('Вы не можете выйти из аккаунта пока заблокированы')
      return
    }
