...
export function SettingsTab({
  ...
  discordRequired: initialDiscordRequired,
  ...
  const [discordRequired, setDiscordRequired] = useState(initialDiscordRequired)
  ...
  const saveDiscordRequired = async (required: boolean) => {
    try {
      const r = await fetch('/api/admin/settings', { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify({ key: 'discordRequired', value: required.toString() }) 
      })
      if (r.ok) { 
        setDiscordRequired(required); 
        notify.success(required ? 'Привязка Discord обязательна' : 'Привязка Discord опциональна') 
      }
    } catch {}
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Discord настройки */}
        <div className="lg:col-span-2 rounded-2xl border border-[#5865F2]/20 bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">Настройки привязки Discord аккаунтов</p>
          
          <div className="grid grid-cols-1 gap-3 mt-4">
            <div className="flex items-center justify-between p-3 rounded-xl bg-accent/50 border border-border">
              <div>
                <p className="text-sm text-foreground">Обязательная привязка Discord</p>
                <p className="text-xs text-muted-foreground">Требовать привязку для покупки/пополнения</p>
              </div>
              <button
                onClick={() => saveDiscordRequired(!discordRequired)}
                className={`relative w-12 h-6 rounded-full transition-colors ${discordRequired ? 'bg-[#5865F2]' : 'bg-muted'}`}
              >
                <span className={`absolute top-1 size-4 rounded-full bg-white transition-transform shadow-sm ${discordRequired ? 'left-7' : 'left-1'}`} />
              </button>
            </div>
          </div>
          ...
          {discordRequired && (
            <div className="mt-3 p-3 rounded-xl bg-[#5865F2]/10 border border-[#5865F2]/20">
              <p className="text-sm text-[#5865F2] font-medium">ℹ️ Пользователи должны привязать Discord перед покупкой</p>
            </div>
          )}
        </div>
      </div>
