        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:400px;">
          ...
          <tr>
            <td style="padding-bottom:32px;text-align:center;">
              <span style="font-size:24px;font-weight:700;color:#fff;letter-spacing:2px;">FLUXOR</span>
            </td>
          </tr>
          <!-- Content -->
          <tr>
          ...
          <tr>
            <td style="padding-top:24px;text-align:center;">
              <span style="font-size:12px;color:#666;">© ${new Date().getFullYear()} Fluxor</span>
            </td>
          </tr>
        </table>
      </td>
...
export async function sendVdsDeletedEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtp.from,
      to: email,
      subject: `VDS ${data.serverName} удалён — Fluxor`,
      html: getEmailTemplate(content),
    })
...
export async function sendVdsCreatedEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtp.from,
      to: email,
      subject: `VDS ${data.serverName} создан — Fluxor`,
      html: getEmailTemplate(content),
    })
...
export async function sendVdsReinstalledEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtp.from,
      to: email,
      subject: `ОС переустановлена на ${data.serverName} — Fluxor`,
      html: getEmailTemplate(content),
    })
...
export async function sendVdsPasswordChangedEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtp.from,
      to: email,
      subject: `Пароль изменён на ${data.serverName} — Fluxor`,
      html: getEmailTemplate(content),
    })
...
export async function sendVmManagerAccountEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtp.from,
      to: email,
      subject: 'Доступ к VMManager — Fluxor',
      html: getEmailTemplate(content),
    })
...
export async function sendServicePaymentConfirmationEmail(
  ...
  try {
    ...
    const content = `
      ...
      <p style="margin:0;font-size:13px;color:#666;text-align:center;">С уважением, Команда Fluxor</p>
    `

    await transporter.sendMail({
      from: smtp.from,
      to: email,
      subject: `Оплата ${data.serviceName} подтверждена — Fluxor`,
      html: getEmailTemplate(content),
    })
