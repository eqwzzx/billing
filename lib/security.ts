...
// ============================================================================
// Rate Limiting
// ВНИМАНИЕ: Используется in-memory хранилище для простоты
// В production окружении с несколькими инстансами необходимо использовать Redis!
// ============================================================================
...
if (!CSRF_SECRET) {
  console.error('CRITICAL: CSRF_SECRET or JWT_SECRET environment variable is required!')
}
...
export function validateWebhookSignature(
  ...
  if (!secret) {
    console.error('[Security] Webhook secret is not configured!')
    return false
  }
...
/**
 * Проверяет timestamp webhook для защиты от replay атак
 * @param timestamp - Unix timestamp в секундах
 * @param maxAgeSeconds - Максимальный возраст webhook в секундах (по умолчанию 5 минут)
 */
export function validateWebhookTimestamp(timestamp: number, maxAgeSeconds: number = 300): boolean {
  const now = Math.floor(Date.now() / 1000)
  const age = now - timestamp
  
  // Проверяем что timestamp не из будущего (с допуском 60 секунд на разницу часов)
  if (age < -60) {
    console.warn('[Security] Webhook timestamp is in the future')
    return false
  }
  
  // Проверяем что webhook не слишком старый
  if (age > maxAgeSeconds) {
    console.warn('[Security] Webhook timestamp is too old:', age, 'seconds')
    return false
  }
